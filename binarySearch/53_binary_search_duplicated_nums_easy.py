"""
初始化： 左边界 i = 0i=0 ，右边界 j = len(nums) - 1j=len(nums)−1 ；代表闭区间 [i, j][i,j] 。
循环二分： 当 i \leq ji≤j 时循环 （即当闭区间 [i, j][i,j] 为空时跳出） ；
计算中点 m = (i + j) // 2m=(i+j)//2 ，其中 "////" 为向下取整除法；
若 nums[m] < targetnums[m]<target ，则数字 targettarget 一定在闭区间 [m + 1, j][m+1,j] 中，因此执行 i = m + 1i=m+1；
若 nums[m] > targetnums[m]>target ，则数字 targettarget 一定在闭区间 [i, m - 1][i,m−1] 中，因此执行 j = m - 1j=m−1；
若 nums[m] = targetnums[m]=target ，则右边界 rightright 在闭区间 [m+1, j][m+1,j] 中；左边界 leftleft 在闭区间 [i, m-1][i,m−1] 中。因此分为以下两种情况：
若查找 右边界 rightright ，则执行 i = m + 1i=m+1 ；（跳出时 ii 指向右边界）
若查找 左边界 leftleft ，则执行 j = m - 1j=m−1 ；（跳出时 jj 指向左边界）

"""

"""
统计一个数字在排序数组中出现的次数。

 

示例 1:

输入: nums = [5,7,7,8,8,10], target = 8
输出: 2
示例 2:

输入: nums = [5,7,7,8,8,10], target = 6
输出: 0
"""

class Solution:
    def search(self, nums: List[int], target: int) -> int:
        # count = 0
        # if not nums:
        #     return count
        # for i in nums:
        #     if i == target:
        #         count+= 1
        # return count

        i, j = 0, len(nums)-1
        while i <= j:
            m = (i+j)//2
            if nums[m] <= target:
                i = m+1
            else:
                j = m-1
        right = i

        i, j = 0, len(nums)-1
        while i <= j:
            m = (i+j)//2
            if nums[m] < target:
                i = m+1
            else:
                j = m-1
        left = j
        return right -left - 1
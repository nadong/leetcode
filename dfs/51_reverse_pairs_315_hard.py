...
# result = [0 for _ in range(len(nums)+1)]
...
# def merge_and_count(nums:List[int], left:int, right:int, mid:int, temp:List[int])-> int:
#     res = 0
...
# def count_reverse_paris(nums: List[int], left: int, right: int,  temp:List[int])-> int:
#     print("nums", nums)
#     if left == right :
#         return 0
#     mid = left + (right - left)//2
#     left_paris = count_reverse_paris(nums, left, mid, temp)
#     right_paris = count_reverse_paris(nums, mid+1, right, temp)
#     reverse_paris = left_paris + right_paris
#     print("reverse_paris", reverse_paris)

#     if nums[mid] <= nums[mid+1]:
#         return reverse_paris

#     reverse_cross_paris = merge_and_count(nums, left, right, mid, temp)

#     return  reverse_paris + reverse_cross_paris

# temp = [0 for _ in range(len(nums))]
# return count_reverse_paris(nums, 0, len(nums)-1, temp)
...
class Solution:
    ...
    def reversePairs(self, nums: List[int]) -> int:
        ...
        # 用于归并的辅助数组
        temp = [0 for _ in range(len(nums))]
        return self.count_reverse_pairs(nums, 0, len(nums) - 1, temp)

    def count_reverse_pairs(self, nums, left, right, temp):
        print("in count Reverse_pairs",nums)
...
class Solution:
    def reversePairs(self, nums: List[int]) -> int:
        ...
        def merge_and_count(nums:List[int], left:int, mid:int, right: int, temp:List[int])-> int:
            for i in range(left, right+1):
                temp[i] = nums[i]
            i = left
            j = mid+1
        ...
        def count_reverse_paris(nums: List[int], left: int, right: int,  temp:List[int])-> int:
            ...
            reverse_cross_paris = merge_and_count(nums, left, mid, right, temp)
            return  reverse_paris + reverse_cross_paris
